classdef DMLP
    ...
    methods
        ...
        function out = getSeed(net, seedNode)
            if nargin == 1
                out = randi(net.netStruct(net.cLayer), 1);
            elseif nargin == 2
                out = net.acVal{net.cLayer}(seedNode);
            end
        end
        ...
        function [outNet, outVal] = forward(net, layer, seed)
            ...
            if nargin == 3
                net.acVal{cLayer} = (net.acVal{pLayer} .* net.dropMap{pLayer} + seed) * net.weight{pLayer} + net.bias{cLayer};
            elseif nargin == 2
                net.acVal{cLayer} = net.acVal{pLayer} * net.weight{pLayer} + net.bias{cLayer};
            end
